    ...
    public async execute(): Promise<EDL> {
        ...
        const allShots: Shot[] = this.plan.script.segments.flatMap((seg, index) => {
            ...
            if (route === 'broll' && !isARoll && segDuration > 4 && (!seg.visuals || seg.visuals.length <= 1)) {
                // Target ~7s per keyframe for relaxed, cinematic pacing
                // Also stays well above LTX minimum (~3.3s) to prevent audio/video mismatch
                const targetShotDuration = 7; // Slower pacing for B-Roll only
